...
一个简单的ThinkPHP Socket 扩展中的Gateway服务
本服务主要负责网络IO

## 安装

~~~
composer require axguowen/think-socket-gateway
~~~

## 配置

首先配置config目录下的socketgateway.php配置文件。
配置项说明：
...
~~~
php think socket:gateway start
~~~

要使用守护进程模式启动可以将配置项deamonize设置为true
或者在启动命令后面追加 -d 参数，如下：
~~~
php think socket:gateway start -d
~~~

### 停止
~~~
php think socket:gateway stop
~~~

### 查看进程状态
~~~
php think socket:gateway status
~~~
