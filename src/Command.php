...
class Command extends Base
{
    ...
    {
        // 指令配置
        $this->setName('socket:gateway')
            ->addArgument('action', Argument::OPTIONAL, 'start|stop|restart|reload|status', 'start')
            ->addOption('daemon', 'd', Option::VALUE_NONE, 'Run the socket gateway service in daemon mode.')
            ->setDescription('Socket Gateway Service For ThinkPHP');
    }
